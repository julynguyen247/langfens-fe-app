...
interface SharedBandRulerProps extends BandRulerProps {
...
}: SharedBandRulerProps) {
  const ticks: number[] = [];
  for (let v = MIN; v <= MAX + 0.0001; v += STEP) {
    ticks.push(Math.round(v * 10) / 10);
  }

  const width = 100; // viewBox units; the SVG scales fluidly via `width="100%"`.
  const height = 40;
  const padX = 1; // leave a sliver of margin so end-tick labels breathe
