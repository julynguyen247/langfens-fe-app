...
type Props = {
  id: string;
  stem: string;
  /** Image URL for the diagram (from DTO imageUrl field). Takes precedence over regex on stem. */
...
const IMG_MD_RE = /!\[([^\]]*)\]\(([^)\s]+)(?:\s+"[^"]*")?\)/;
const PART_RE = /\[Diagram:\s*([^\]]+?)\s*\]/i;

/**
 * DIAGRAM_LABEL — renders a real diagram image (when the prompt carries
 * one) above a numbered list of letter-inputs. Falls back to a single
 * input if the prompt doesn't expose a part list. Specialised counterpart
 * of MapLabelCard (which matches `[Map: ...]` markers).
 */
const DiagramLabelCard = memo(function DiagramLabelCard({
  id,
  stem,
...
export default DiagramLabelCard;
